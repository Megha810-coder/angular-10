...
@Component({
  ...
  styleUrls: ['./principle-login.component.css']
})
export class PrincipleLoginComponent implements OnInit {
  ...
  UserLogin(){
       ...
       this.userService.savePrincipleLogin(this.user).subscribe(
        response =>{
          let result = response;
  ...
  UserEmail(){
    ...
    this.userService.sendPrincipleOtp(this.user).subscribe(
  ...
  newPass(){
    ...
    if(pass==confirmpass){
      ...
      this.user.email= this.Email?.value;
      this.user.password = this.Password?.value;
      this.userService.setNewPrinciplePass(this.user).subscribe(
        response =>{
          if(response<=0){
